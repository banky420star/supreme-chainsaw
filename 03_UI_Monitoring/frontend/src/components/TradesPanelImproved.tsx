/**
 * TradesPanel Component - IMPROVED VERSION
 *
 * Displays trading activity, performance metrics, and trade history.
 * Includes explanatory tooltips to help users understand the system.
 *
 * KEY CONCEPTS EXPLAINED:
 * - PnL (Profit/Loss): The gain or loss from a trade
 * - Win Rate: Percentage of profitable trades
 * - Profit Factor: Gross profit / Gross loss (higher is better, >1.0 = profitable)
 * - Drawdown: Peak-to-trough decline in equity
 * - Economic Calendar: High-impact news events that may affect trading
 */
...
// Equity chart component - visualizes account balance over time
...
// Help text explanations for metrics
const HELP_TEXTS = {
  totalTrades: "Total number of trades executed by the system. Includes both winning and losing trades.",
  wins: "Number of profitable trades. A trade is considered a win if it closes with positive PnL.",
  losses: "Number of unprofitable trades. A trade is considered a loss if it closes with negative PnL.",
  winRate: "Percentage of trades that were profitable. Formula: (Wins / Total Trades) × 100. Higher is better, but quality matters more than quantity.",
  pnl: "Profit/Loss - The total net gain or loss from all trades combined. Positive = profit, Negative = loss.",
  profitFactor: "Gross Profit / Gross Loss. >1.0 means profitable overall. >1.5 is considered good. >2.0 is excellent.",
  equityCurve: "Visual representation of account balance over time. Shows the growth (or decline) of your trading account.",
  drawdown: "Maximum peak-to-trough decline in equity. Lower is better. 8% means the account dropped 8% from its highest point.",
  economicCalendar: "High-impact economic events that can cause market volatility. The system may pause trading before major news (like Fed announcements).",
  outcome: "Trade result: Win (profitable), Loss (unprofitable), or Breakeven.",
}
...
const TradesPanelImproved: React.FC<Props> = ({ calendar }) => {
  ...
  return (
    <div style={{ background: colors.bg, color: colors.text, padding: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        <HelpTooltip
          title="Trading Activity Panel"
          text="This panel shows your trading performance metrics, equity curve, and trade history. Use this to monitor how the AI is performing."
        />
      </div>

      {/* KPIs with help tooltips */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 12, marginBottom: 16 }}>
        {[
          { label: 'Total', value: ov?.total_trades ?? '--', help: HELP_TEXTS.totalTrades },
          { label: 'Wins', value: ov?.wins ?? '--', help: HELP_TEXTS.wins },
          { label: 'Losses', value: ov?.losses ?? '--', help: HELP_TEXTS.losses },
          { label: 'Win Rate', value: ov?.win_rate != null ? `${(ov.win_rate * 100).toFixed(1)}%` : '--', help: HELP_TEXTS.winRate },
          { label: 'PnL', value: ov?.total_pnl != null ? `$${fmtMoney(ov.total_pnl)}` : '--', help: HELP_TEXTS.pnl },
          { label: 'Profit Factor', value: ov?.profit_factor != null ? String(typeof ov.profit_factor === 'number' ? ov.profit_factor.toFixed(2) : ov.profit_factor) : '--', help: HELP_TEXTS.profitFactor },
        ].map((kpi) => (
          <div key={kpi.label} style={{ ...panelStyle, textAlign: 'center', position: 'relative' }}>
        ...
        <div style={{ ...panelStyle, marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <HelpTooltip text={HELP_TEXTS.equityCurve} />
            </div>
            <div style={{ display: 'flex', gap: 12, fontSize: 11, fontFamily: 'monospace' }}>
              ...
              <HelpTooltip text={HELP_TEXTS.drawdown} />
            </div>
          </div>
      ...
      {calendar.length > 0 && (
        <div style={{ ...panelStyle, marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <h3 style={{ margin: 0, fontSize: 12, color: colors.muted, fontWeight: 600 }}>Economic Calendar (next 7 days)</h3>
            <HelpTooltip text={HELP_TEXTS.economicCalendar} />
          </div>
      ...
      {!loading && trades.length === 0 && (
        <div style={{ ...panelStyle, color: colors.muted }}>
          No trades found. This is normal when starting fresh - the AI waits for high-confidence signals.
        </div>
      )}
                ...
                <tr>
                  ...
                  <th style={thStyle}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      Outcome
                      <HelpTooltip text={HELP_TEXTS.outcome} />
                    </span>
                  </th>
...
export default TradesPanelImproved
