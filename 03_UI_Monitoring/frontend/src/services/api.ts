...
/* ─── Decision PPO + Execution Rich Telemetry (Observability Completion Agent) ─── */
export async function fetchExecutionDecisions(limit = 15): Promise<any> {
  const r = await fetch(`${BASE}/api/execution/decisions?limit=${limit}`, { cache: 'no-store' })
  return r.ok ? r.json() : { decisions: [], count: 0 }
}

export async function fetchExecutionLive(): Promise<any> {
  const r = await fetch(`${BASE}/api/execution/live`, { cache: 'no-store' })
  return r.ok ? r.json() : { status: 'offline', active: [] }
}

export async function fetchExecutionFeedback(limit = 20): Promise<any> {
  const r = await fetch(`${BASE}/api/execution/feedback?limit=${limit}`, { cache: 'no-store' })
  return r.ok ? r.json() : { feedback: [] }
}

/* ─── Timing Analyzer Insights (Observability & Dreamer/Rainforest Timing Integration) ─── */
export async function fetchTimingInsights(): Promise<any> {
  const r = await fetch(`${BASE}/api/timing/insights`, { cache: 'no-store' })
  return r.ok ? r.json() : { error: 'unavailable', source: 'none' }
}
