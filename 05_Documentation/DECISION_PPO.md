...
### Continual / Online Adaptation (2026-05-28)
The Decision PPO policy head (drl/decision_head.py) now supports safe lightweight online updates via Python/autonomous/continual_learner.py:
- Uses live/paper feedback + high-value ExperienceMemory replay (IS weights).
- Tiny PPO surrogate + EWC updates on the rich 18-dim DecisionHead.
- Gated, drift-tracked, respects meta overrides from XAU artifacts.
- Orchestrator decides light continual vs full retrain; supervisor runs periodically.
See continual_learner_complete.json for full wiring + smoke results. TUI shows adaptation status.
