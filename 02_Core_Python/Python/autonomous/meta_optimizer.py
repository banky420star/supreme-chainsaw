...
class MetaOptimizer:
    ...
    def _get_current_regime(self) -> str:
        """Uses FULL RegimeAdaptiveController (primary) for accurate bull/bear/ranging/breakout + sub_regime."""
        try:
            from Python.autonomous.regime_controller import get_regime_controller
            ctrl = get_regime_controller()
            for sym, st in getattr(ctrl, "_last_states", {}).items():
                if st and getattr(st, "regime", None):
                    return str(st.regime)
            status = ctrl.get_current_status() if hasattr(ctrl, "get_current_status") else {}
            regimes = status.get("current_regimes", {}) or {}
            if regimes:
                return list(regimes.values())[0]
        except Exception:
            pass
        if self.rainforest:
            try:
