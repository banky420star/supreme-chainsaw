...
class AutonomousRetrainingOrchestrator:
    ...
    def run_cycle(self, force: bool = False):
        ...
        Now distinguishes 'light online continual' vs 'full retrain' (requirement integration).
        """
        ...
        light_done = False
        if self.config.enable_continual_learner and ContinualLearner is not None:
            trade_count = metrics.get("trade_count", 0)
            # Light online decision heuristic (stable, practical): few new trades OR mild signal, no full degradation
            mild_signal = (trade_count > 5 and trade_count < 35) or ("high_value" in reason.lower() and trade_count < 80)
            if mild_signal and not needed:
                try:
                    cl = ContinualLearner(ContinualConfig(symbol=self.config.symbols[0] if self.config.symbols else "XAUUSDm"))
                    cl_res = cl.run_online_update_cycle(force=False)
                    light_done = any(v.get("applied") for v in cl_res.get("updates", {}).values())
                    self.log("light_continual_update", {
                        "applied": light_done,
                        "drift": cl_res.get("drift", {}),
                        "status_path": "runtime/agent_status/continual_learner_complete.json",
                    })
                    if light_done:
                        self.state["last_light_online"] = datetime.now(timezone.utc).isoformat()
                        # Do not launch heavy full jobs
                        self.state["last_check"] = datetime.now(timezone.utc).isoformat()
                        self._write_status("light_online_continual_complete", {"light_update": cl_res.get("drift", {})})
                        return {"retrained": False, "light_online": True, "reason": "continual_light_update_triggered"}
                except Exception as cl_e:
                    self.log("light_continual_error", {"err": str(cl_e)[:160]})
        ...
        return {"retrained": needed or force, "light_online": light_done, "reason": reason}
