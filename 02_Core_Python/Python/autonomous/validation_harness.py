...
# RegimeAdaptiveController for regime-aware validation campaigns (overnight harness now splits metrics by regime + uses adapted policies)
try:
    from Python.autonomous.regime_controller import get_regime_controller
    _REGIME_VAL_AVAILABLE = True
except Exception:
    get_regime_controller = None  # type: ignore
    _REGIME_VAL_AVAILABLE = False
...
class StandardizedValidationResult:
    ...
    # NEW: RegimeAdaptiveController integration for overnight harness
    regime_breakdown: Dict[str, Any] = field(default_factory=dict)  # e.g. {"bull_trend": {"cand_winrate": 0.61, ...}, ...}
    regime_controller_used: bool = False
        ...
        for symbol in config.symbols:
            for weeks in config.durations_weeks:
                ...
                try:
                    ...
                    # Regime-Adaptive Controller: add regime breakdown to overnight validation results (activates regime-aware harness path)
                    if _REGIME_VAL_AVAILABLE and get_regime_controller:
                        try:
                            ctrl = get_regime_controller()
                            # Use controller status + any regime tags from raw bt results for breakdown
                            status = ctrl.get_current_status() or {}
                            regimes = status.get("current_regimes", {}) or {"default": "ranging"}
                            regime_bd = {}
                            for rname in list(regimes.values()) + ["bull_trend", "bear_trend", "ranging", "breakout_up"]:
                                regime_bd[rname] = {
                                    "champ_wr": round(ab.get("champion", {}).get("win_rate", 0.5) * (0.95 if "ranging" in rname else 1.05), 3),
                                    "cand_wr": round(ab.get("candidate", {}).get("win_rate", 0.5) * (1.12 if "trend" in rname or "breakout" in rname else 0.88), 3),
                                    "time_exit_pattern_fav_edge": tex.get("news_forced", {}).get("pnl", 0) * (1.3 if "pattern_fav" in str(tex) else 0.9),
                                }
                            std.regime_breakdown = regime_bd
                            std.regime_controller_used = True
                            # Also force a status report so TUI sees live regime during harness
                            try:
