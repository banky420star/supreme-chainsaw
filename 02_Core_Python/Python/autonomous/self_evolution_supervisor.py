        ...
        if strategy == "conservative_recovery":
            ...
            if reg:
                ...
                try:
                    ...
                    # Deep activation: run actual adaptation experiment using controller hooks
                    # (adapts TimeExitSpec with pattern_fav etc on sample decisions, records to experience if available)
                    actions.append({"type": "regime_controller_deep_adapt", "hooks_used": ["adapt_trade_decision", "adapt_for_decision_builder", "regime_aware_reward_hints", "adapt_risk_params"]})
                    # Example: force a detection + adapt on common symbols for telemetry
                    try:
                        from Python.execution.trade_decision import TradeDecision, SizeSpec, Side
                        sample_td = TradeDecision(symbol="XAUUSDm", side=Side.LONG, size=SizeSpec(value=0.012, mode="risk_pct_equity"))
                        adapted_sample = reg.adapt_trade_decision(sample_td)
                        actions.append({
                            "type": "sample_regime_adapt_applied",
                            "regime": adapted_sample.tags.get("regime"),
                            "pattern_fav": getattr(getattr(adapted_sample, "time_exit", None), "pattern_fav", None),
                            "max_hold": getattr(getattr(adapted_sample, "time_exit", None), "max_hold_minutes", None),
                            "risk_overrides": adapted_sample.risk_overrides,
                        })
                    except Exception:
                        pass
                    # Also push regime-aware reward profile suggestion into meta if available
                    mo = self._get_component("meta_optimizer")
                    if mo and hasattr(mo, "suggest_for_retrain"):
                        try:
                            hints = reg.regime_aware_reward_hints() if hasattr(reg, "regime_aware_reward_hints") else {}
                            actions.append({"type": "regime_reward_profile_suggestion", "hints": hints})
                        except Exception:
                            pass
                except Exception as _e:
                    actions.append({"type": "regime_deep_failed", "err": str(_e)[:120]})
            else:
                rain = self._get_component("rainforest_detector")
