...
# RegimeAdaptiveController full activation: policy adapter + regime-aware TimeExitSpec (pattern_fav etc) in backtests
try:
    from Python.autonomous.regime_controller import get_regime_controller
    _REGIME_BT_AVAILABLE = True
except Exception:
    get_regime_controller = None  # type: ignore
    _REGIME_BT_AVAILABLE = False
    Side = None
    ExitSpec = None
    ExitType = None
    TrailingSpec = None
...
def make_pattern_timing_candidate_policy():
    ...
    def _cand(obs: Dict[str, Any], **kw) -> Optional["TradeDecision"]:
        ...
        td = TradeDecision(
            ...
            time_exit=TimeExitSpec(max_hold_minutes=hold, close_before_high_impact_news=close_news, close_at_session_end=True, pattern_fav=favorable),
            pattern_context=pat,
            timing_context=tim,
            confidence=conf,
        )

        # RegimeAdaptiveController activation: adapt the TD (applies full regime TimeExitSpec overrides incl. pattern_fav, risk caps, vol_target, trailing/partials from detected Rainforest+Pattern state)
        if _REGIME_BT_AVAILABLE and get_regime_controller:
            try:
                ctrl = get_regime_controller()
                # Detect on the fly if recent_df in obs/kw for real regime (otherwise uses last or defaults to adaptation)
                df_for_regime = kw.get("recent_df") or obs.get("recent_df")
                if df_for_regime is not None and hasattr(df_for_regime, "__len__") and len(df_for_regime) > 40:
                    try:
                        st = ctrl.detect_regime(td.symbol, df_for_regime.tail(120) if hasattr(df_for_regime, "tail") else df_for_regime)
                        td = ctrl.adapt_trade_decision(td, state=st)
                    except Exception:
                        td = ctrl.adapt_trade_decision(td)
                else:
                    td = ctrl.adapt_trade_decision(td)
            except Exception:
                pass
