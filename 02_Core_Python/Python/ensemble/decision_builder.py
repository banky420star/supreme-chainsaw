...
# Regime-Adaptive Controller integration (full activation)
try:
    from Python.autonomous.regime_controller import get_regime_controller
    _REGIME_CTRL_AVAILABLE = True
except Exception:
    get_regime_controller = None  # type: ignore
    _REGIME_CTRL_AVAILABLE = False
...
class DecisionBuilder:
    ...
    def build_intent(
        ...
        # Regime-Adaptive Controller hook: overrides / enriches with full TimeExitSpec (pattern_fav, news_close, session_close, max_hold, vol/risk)
        ctrl_hints = {}
        if _REGIME_CTRL_AVAILABLE and get_regime_controller:
            try:
                ctrl = get_regime_controller()
                ctrl_hints = ctrl.adapt_for_decision_builder(
                    symbol=symbol,
                    raw_votes=raw_votes,
                    timing_context=t_ctx,
                    pattern_context=pat_ctx,
                ) or {}
                te_h = ctrl_hints.get("time_exit_hints", {})
                if te_h:
                    time_hints.update({
                        "close_before_high_impact_news": te_h.get("close_before_high_impact_news", time_hints.get("close_before_high_impact_news", True)),
                        "close_at_session_end": te_h.get("close_at_session_end", False),
                        "max_hold_minutes": te_h.get("max_hold_minutes", 120),
                        "pattern_fav": te_h.get("pattern_fav", favorable),  # controller's pattern_fav drives behavior
                        "vol_target_scale": te_h.get("vol_target_scale", 1.0),
                    })
                # Apply controller size/risk directly to exposure
                if ctrl_hints.get("size_scale"):
                    target_exposure *= float(ctrl_hints["size_scale"])
                if ctrl_hints.get("risk_cap_pct"):
                    target_exposure = min(target_exposure, float(ctrl_hints["risk_cap_pct"]) * 12)  # rough %->exposure map
            except Exception as _e:
                logger.debug(f"DecisionBuilder regime controller hook failed gracefully: {_e}")

        if favorable or time_hints.get("pattern_fav"):
            ...
            target_exposure = min(self.MAX_EXPOSURE_PCT, target_exposure * 1.22)
            max_hold = int(self.max_hold_bars * 1.35)
            tp_atr = self.take_profit_atr * 1.18
            time_hints.setdefault("close_before_high_impact_news", False)
            time_hints.setdefault("max_hold_minutes", max(150, time_hints.get("max_hold_minutes", 150)))
            time_hints.setdefault("partials_aggressive", True)
            if d_sim.get("simulated_reward", 0) > 0.8:
                tp_atr *= 1.1
        else:
            # Caution: tight time exit, smaller size already handled
            time_hints.setdefault("close_before_high_impact_news", bool(t_ctx.get("news_proximity", 0) > 0.3))
            time_hints.setdefault("max_hold_minutes", 70)
        ...
        logger.debug(f"DecisionBuilder emitted intent {intent.intent_id} side={intent.side} exp={intent.target_exposure_pct} pattern_fav={time_hints.get('pattern_fav', favorable)} regime_ctrl={'yes' if ctrl_hints else 'no'}")
        return intent
