...
# When no model bundle exists, we need 150 features to match build_feature_frame()
FEATURE_COLUMNS = list(ENGINEERED_LSTM_COLUMNS)  # 17-col legacy; overwritten below for fresh model

# Override: if no trained model exists, use 150 features to match ULTIMATE_150
# This ensures the fresh AGIModel matches what build_feature_frame() produces
try:
    _sample_df = __import__('pandas').DataFrame({
        "time": __import__('pandas').date_range("2026-01-01", periods=10, freq="5min", tz="UTC"),
        "open": [1.0]*10, "high": [1.01]*10, "low": [0.99]*10,
        "close": [1.0]*10, "volume": [100]*10
    })
    _sample_features, _ = build_lstm_feature_frame(_sample_df, feature_version=ULTIMATE_150)
    FEATURE_COLUMNS = list(_sample_features.columns)
except Exception:
    pass  # Keep default 17 if anything fails
