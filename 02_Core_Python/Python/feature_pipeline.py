...
# Sentinel to suppress repeated warnings when PatternDetector is unavailable
class _Sentinel:
    flag = False
_pattern_detector_warned = _Sentinel()
...
def _build_engineered_env_matrix(df: pd.DataFrame) -> np.ndarray:
    ...
    if _PATTERN_DETECTOR_AVAILABLE and n >= 5:
        try:
            ...
            # PatternDetector unavailable or failed — pattern block stays zero.
            # This means Dreamer + Decision PPO run WITHOUT pattern context.
            # Log once to aid diagnosis; suppressed after first failure.
            if not getattr(_pattern_detector_warned, "flag", False):
                logger.warning(f"PatternDetector failed, pattern context disabled: {exc}")
                _pattern_detector_warned.flag = True  # type: ignore
...
# ============================================================
# Multi-Timeframe per Symbol (1m + 5m + 15m + 1h)
# ============================================================
# Note: the underlying builder is imported at the top of this module.
# build_multitimeframe_features, load_best_feature_params, and
# get_multitimeframe_feature_count are available via:
...
def build_env_feature_matrix(df: pd.DataFrame, feature_version: str = ENGINEERED_V2) -> np.ndarray:
    if _is_multitimeframe_best_request(feature_version):
        ...
        base = _original_build_env(df, ENGINEERED_V2)
        return _apply_artifact_feature_importance_scaling(base, feature_version)
    base = _original_build_env(df, feature_version)
    return _apply_artifact_feature_importance_scaling(base, feature_version)
