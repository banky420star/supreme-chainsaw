...
# NOTE: Do NOT import _normalize_ohlcv or other feature_pipeline items at module level.
# feature_pipeline imports this module at the top, so a top-level import here would
# create a circular import. Use lazy imports inside functions that need them.
# (feature_pipeline is imported lazily inside build_multitimeframe_features below.)
...
def build_multitimeframe_features(
    ...
    def add_feats(df: pd.DataFrame, prefix: str = "") -> pd.DataFrame:
        # Lazy import to avoid circular dependency with feature_pipeline
        from Python.feature_pipeline import _normalize_ohlcv
