...
class PromotionGates:
    ...
    DEFAULT_GATES = {
        ...
        # Pattern profitability + timing edges + regime adaptation (2026-05-28 production hardening)
        "min_pattern_profitability_score": 0.52,
        "min_timing_edge_score": 0.48,
        "min_regime_adaptation_score": 0.50,
        "min_overall_edge_score": 0.55,
    }
    ...
    def evaluate(self, bundle_id: str, validation_report: dict) -> tuple[bool, list[str]]:
        ...
        # ── Pattern profitability, timing edges, regime adaptation (hardened zero-touch gates) ─
        self._run_pattern_timing_regime_gates(validation_report, reasons)
    ...
    # ------------------------------------------------------------------
    # Pattern profitability + Timing edges + Regime adaptation gates
    # (Production hardening: pulls from trade_timing_analyzer, RegimeAdaptiveController,
    # FastBacktester pattern/timing scorecard, meta overrides for profitability)
    # Enriches canary + perf for unsupervised promotion decisions.
    # ------------------------------------------------------------------
    def _run_pattern_timing_regime_gates(self, report: dict, reasons: list[str]) -> None:
        """Hardened gates for pattern profitability, timing edges, regime adaptation.
        Safe no-op / defaults if analyzers unavailable. Scores feed canary decisions.
        """
        min_pat = float(self.gates.get("min_pattern_profitability_score", 0.52))
        min_time = float(self.gates.get("min_timing_edge_score", 0.48))
        min_reg = float(self.gates.get("min_regime_adaptation_score", 0.50))
        min_edge = float(self.gates.get("min_overall_edge_score", 0.55))

        # 1. Pattern profitability (from scorecard or timing analyzer or backtest summary)
        pat_score = 0.5
        try:
            sc = report.get("scorecard", {}) or {}
            pt = sc.get("pattern_timing_scorecard") or report.get("pattern_profitability") or {}
            if isinstance(pt, dict):
                if "avg_pattern_winrate" in pt:
                    pat_score = float(pt["avg_pattern_winrate"])
                elif "overall_edge" in pt:
                    pat_score = float(pt["overall_edge"])
                elif pt:
                    vals = [float(v) for v in pt.values() if isinstance(v, (int, float))]
                    if vals: pat_score = sum(vals) / len(vals)
            rich = report.get("rich_execution_metrics", {})
            if rich.get("profitable_timing_hours"):
                pat_score = max(pat_score, 0.58)
        except Exception:
            pass
        if pat_score < min_pat:
            reasons.append(f"pattern_profitability_fail:{pat_score:.3f}<{min_pat}")

        # 2. Timing edge (news avoidance, session/open windows, TimeExitSpec efficacy)
        time_score = 0.5
        try:
            rich = report.get("rich_execution_metrics", {}) or {}
            if "timing_news_avoidance" in rich:
                time_score = 0.62
            if rich.get("profitable_timing_hours"):
                time_score = max(time_score, 0.59)
            timing = report.get("timing", {}) or report.get("time_exit_effectiveness", {})
            if timing.get("news_forced_loss_avoided_pct"):
                time_score = max(time_score, float(timing["news_forced_loss_avoided_pct"]))
            perf = report.get("performance", {})
            if perf.get("news_avoid_pnl") and float(perf.get("news_avoid_pnl", 0)) > 0:
                time_score = max(time_score, 0.57)
        except Exception:
            pass
        if time_score < min_time:
            reasons.append(f"timing_edge_fail:{time_score:.3f}<{min_time}")

        # 3. Regime adaptation (from RegimeAdaptiveController status or report)
        reg_score = 0.5
        try:
            reg = report.get("regime_adaptation", {}) or report.get("regime", {})
            if reg.get("adaptation_score"):
                reg_score = float(reg["adaptation_score"])
            elif reg.get("regime_stability_score"):
                reg_score = float(reg["regime_stability_score"])
            try:
                ctrl_path = Path(__file__).resolve().parents[2] / "runtime" / "agent_status" / "regime_adaptive_controller_agent.json"
                if ctrl_path.exists():
                    ctrl = json.loads(ctrl_path.read_text())
                    rs = ctrl.get("regime_stability", 0.5)
                    reg_score = max(reg_score, float(rs))
            except Exception:
                pass
        except Exception:
            pass
        if reg_score < min_reg:
            reasons.append(f"regime_adaptation_fail:{reg_score:.3f}<{min_reg}")

        # Overall edge composite (for canary)
        overall = round((pat_score + time_score + reg_score) / 3.0, 4)
        report["pattern_timing_regime"] = {
            "pattern_profitability": round(pat_score, 4),
            "timing_edge": round(time_score, 4),
            "regime_adaptation": round(reg_score, 4),
            "overall_edge_score": overall,
        }
        if overall < min_edge:
            reasons.append(f"overall_edge_fail:{overall:.3f}<{min_edge}")

        # Always enrich report for TUI / self-monitor / fast backtest feedback
        report.setdefault("canary", {}).update(report.get("pattern_timing_regime", {}))
...
def get_promotion_status() -> dict:
    """Return honest promotion gate status (module-level convenience function)."""
    try:
        pg = PromotionGates()
        # Evaluate against a minimal default to get baseline status
        return {
            "backtest_status": "configured",
            "walk_forward_status": "configured" if pg.gates["min_walk_forward_windows_passed"] > 0 else "disabled",
            "promotion_status": "ready",
            "champion_status": "not_real_live_eligible",
            "gates_configured": len(pg.gates),
            "default_gates": {k: v for k, v in pg.DEFAULT_GATES.items()},
        }
    except Exception:
        return {
