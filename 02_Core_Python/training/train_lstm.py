...
from training.progress_writer import update_training_progress, mark_training_completed, mark_training_failed
...
def train_lstm(
    ...
    update_training_progress("lstm", {"running": True, "current_symbol": symbols[0] if symbols else None})
    ...
    if not results:
        ...
        try:
            ...
            pass
        mark_training_failed("No symbol models trained")
        return
    ...
    mark_training_completed(
        symbol=best['symbol'],
        final_metrics={"loss": best['loss'], "samples": best['samples']}
    )
