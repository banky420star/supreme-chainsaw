...
.DESCRIPTION
    ...
    Default: React dev server (hot reload + proxy to 5050 for complete data).
...
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host " React UI (frontend/) + api_server (5050) + Supervisor (9090) + TUI" -ForegroundColor Green
Write-Host ""
...
if (-not $NoSupervisor) {
    ...
    if (Test-Path $supPath) {
        Write-LaunchLog 'INFO' 'Starting vps_agi_supervisor (background)'
...
if (-not $NoTui) {
    ...
    if (Test-Path $tuiL) {
        Start-Tracked -FilePath 'powershell.exe' -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',"`"$tuiL`"",'-Watcher','-Persistent') -LogFile (Join-Path $LogsDir 'tui_via_launcher.log') -Title 'TUI' -NoWindow
    }
}
...
Write-Host "  FULL STACK LAUNCHED - React Monitoring UI Ready" -ForegroundColor Green
Write-Host "================================================================" -ForegroundColor Green
